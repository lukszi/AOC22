...
def draw_formations(formations: List[List[tuple[int, int]]]) -> ndarray:
    ...
    extend_left = abs(min(0, drop_x - sim.shape[1] + 2))
    extend_right = abs(min(0, sim.shape[0] - (drop_x + sim.shape[1] + 2)))
    sim = np.hstack((np.zeros((sim.shape[0], extend_left), dtype=int), sim, np.zeros((sim.shape[0], extend_right), dtype=int)))

    sim = np.vstack((sim, np.zeros((1, sim.shape[1]), dtype=int)))
    sim = np.vstack((sim, np.ones((1, sim.shape[1]), dtype=int)))
...
def simulate_sand_drop(sim: ndarray):
    ...
    coord = (0, drop_x)
    if sim[coord] == SAND:
        raise ValueError("Sandcorn already present")
    sim[coord] = SAND
